...
async function handleApi(request, response, pathname) {
  if (pathname === "/api/status") {
    return sendJson(response, 200, {
      ...
      message: "Beta 1.2 local launcher is connected."
    });
  }
...
async function start() {
  for (let offset = 0; offset < 20; offset += 1) {
    ...
    try {
      ...
      console.log(`Lyrian Beta 1.2 local launcher running at ${url}`);
      console.log("Close this terminal window to stop the local server.");
      openBrowser(url);
      return;
    } catch (error) {
